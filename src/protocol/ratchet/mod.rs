...
/// Bumped to 6 when the SPQR combiner moved from a root-key fold to Signal's
/// per-message `KDF_HYBRID`: the SPQR state shape changed (per-epoch KDF chains +
/// skipped-key store) and the header replaced the fold-epoch fields with the PQ
/// counter. v5 (and earlier) sessions are rejected by `deserialize` and must rekey.
